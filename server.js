...
app.get('/api/session/:code/public', (req, res) => {
  const session = stmts.getSessionByCode.get(req.params.code);
  if (!session) return res.status(404).json({ success: false, message: '场次不存在' });
  if (session.status === 'ended') return res.json({ success: false, message: '该场次已结束' });
  const groups = stmts.getGroups.all(session.id);
  const groupsWithRoles = groups.map(g => {
...
// 许言选择公开/保留
app.post('/api/session/:code/xu-yan', (req, res) => {
  const session = stmts.getSessionByCode.get(req.params.code);
  if (!session) return res.status(404).json({ success: false, message: '场次不存在' });
  const { groupId, choice } = req.body;
  if (!groupId || !choice) return res.status(400).json({ success: false, message: '缺少参数' });
  const groups = stmts.getGroups.all(session.id);
  const idx = groups.findIndex(g => g.id === groupId);
  if (idx < 0) return res.status(404).json({ success: false });
  // 更新该组的许言选择
  const groupData = groups[idx];
  groupData.xuYanChoice = choice;
  db.prepare('UPDATE groups_t SET name = name WHERE id = ?').run(groupId);
  io.to('session:' + req.params.code).emit('xu_yan_chosen', { groupId, choice });
  res.json({ success: true });
});

// 提交最终决策
app.post('/api/session/:code/decision', (req, res) => {
  const session = stmts.getSessionByCode.get(req.params.code);
  if (!session) return res.status(404).json({ success: false, message: '场次不存在' });
  const { groupId, roleName, keep, rules, choice, words } = req.body;
  if (!groupId || !keep || !choice) return res.status(400).json({ success: false, message: '缺少必填项' });
  // Store decision
  db.prepare(`CREATE TABLE IF NOT EXISTS decision_records (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    session_id INTEGER NOT NULL,
    group_id INTEGER NOT NULL,
    role_name TEXT,
  ...
  db.prepare('INSERT INTO decision_records (session_id, group_id, role_name, keep, rules, choice, words) VALUES (?,?,?,?,?,?,?)')
    .run(session.id, groupId, roleName, keep, JSON.stringify(rules||[]), choice, words||'');
  io.to('session:' + req.params.code).emit('decision_submitted', { groupId, roleName, choice, words });
  res.json({ success: true, message: '决策已提交' });
});

// 获取所有决策
app.get('/api/session/:code/decisions', (req, res) => {
  const session = stmts.getSessionByCode.get(req.params.code);
  if (!session) return res.status(404).json({ success: false, message: '场次不存在' });
  try {
    const decisions = db.prepare('SELECT * FROM decision_records WHERE session_id = ? ORDER BY group_id').all(session.id);
    res.json({ success: true, decisions: decisions.map(d => ({ ...d, rules: JSON.parse(d.rules||'[]') })) });
  } catch(e) { res.json({ success: true, decisions: [] }); }
});

// 更新场景（标题、内容、对白、任务、角色内容等）
app.put('/api/session/:code/scene/:sceneId', (req, res) => {
  const session = stmts.getSessionByCode.get(req.params.code);
  if (!session) return res.status(404).json({ success: false, message: '场次不存在' });
  const { title, content, round_type, full_dialogue, task_content, roleContent } = req.body;
  // 更新场景主表
  db.prepare('UPDATE scenes SET title = ?, content = ?, round_type = ?, full_dialogue = ?, task_content = ? WHERE id = ?').run(
    title || '', content || '', round_type || 'script', full_dialogue || '', task_content || '', parseInt(req.params.sceneId)
  );
  // 更新角色专属内容
  if (roleContent && typeof roleContent === 'object') {
    for (const [roleId, txt] of Object.entries(roleContent)) {
      if (txt && txt.trim()) {
        stmts.addSceneRoleContent.run(parseInt(req.params.sceneId), parseInt(roleId), txt);
      }
    }
  }
  ...
  io.to('session:' + req.params.code).emit('scene_updated', { sceneId: parseInt(req.params.sceneId) });
  res.json({ success: true });
});
...
app.post('/api/preset/salon-621', (req, res) => {
  ...
  const roleDefs = [
    { name: '周岚', desc: '主理人 · 店名是她起的，12年没改过 · 不是不让改，是怕改到最后店不再是这家店' },
    { name: '林澈', desc: '推动者 · 2018年被陈栩请过一杯咖啡 · 店必须动起来，但不能保持沉默' },
    { name: '阿宁', desc: '老员工 · 周岚的温水让她留下来 · 很多事能做，别总临时通知' },
    { name: '许言', desc: '撰稿人 · 表妹曾被铁盒救助 · 外部资源解决不了内部问题' },
  ];
  ...
  insertScene(1, '第1环节：开场 · 角色认领', '老城区梧桐街拐角，一家开了十二年的小店——621号店。\n\n木质招牌被风雨磨得旧了，但那盏暖黄的灯一亮，整条街都跟着安静。十二年来，有人在这里准备过考试，有人在这里表白，有人失恋后独坐一整晚，也有人把第一份工作offer的消息带到这里分享。\n\n明天，房东就要来问最后决定了：续租、转型、还是关门。今晚，所有和这家店命运相关的人被叫了回来。', 'script', '', '', {
 '周岚': '你是周岚，42岁，621号店的主理人。店名是你起的，十二年来从没改过。\n\n十二年前，陈栩拽着你走进这间空铺子。你说"咖啡机都买不起"，她说"先把门开了再说"。那是你人生里第一次被一个人推着往前走。\n\n2018年陈栩走了。那个推过你的人最后觉得你走不动了。你一个人撑了六年。\n\n我不是不让大家提方案，但这家店不是说改就改的。',
 '林澈': '你是林澈，29岁，近两年才深度参与店里事务。\n\n2018年秋天，你21岁，人生最低谷时走进这家店。吧台后面是陈栩。她请你喝了一杯咖啡，说"不知道就不知道，先把这杯喝完"。\n\n六年后你回来，吧台后换成了周岚。你问"之前还有一个人"，她没有接话。你帮这家店做了两年事，是替那个不在这里的人守着。\n\n店必须动起来。先往前走，比一直卡着强。',
 '阿宁': '你是阿宁，34岁，621号店的老员工。\n\n四年前你经历了一段很差的日子，每天不知道去哪里。周岚每天在你桌上放一杯温水。有一天她问你"明天还来吗"，你第二天真的来了。\n\n你来这家店不是因为招工启事，是因为那杯水。你记了一整本笔记——不是因为记仇，是因为你在乎这家店怎么运转。\n\n很多事不是不能做，是最后都变成临时来。',
 '许言': '你是许言，38岁，本地独立撰稿人。\n\n五年前，你表妹收到一笔以"621号店"名义捐出的救助款。你找到这家店，周岚指了指吧台角落一个旧铁盒说"铁盒里刚好够"。这家店每卖一杯咖啡留一点，悄悄做了快十年。\n\n你写了那篇三千字报道，是在替一个受助过的家庭回一句谢谢。今晚你来，是看那个铁盒还在不在吧台角落。\n\n外部资源解决不了内部长期卡住的问题。',
});
  insertScene(2, '第2环节 · 第1幕：灯还亮着', '', 'script', '**周岚**（手指轻轻压着桌上的本子，语气尽量平稳，但能听出疲惫）：\n「人都到齐了吧。那我直说了——明天房东要来，要我们给一个最后答复。续租、转型、还是关门。今晚必须定下来。」\n（她停了停，目光扫过所有人）\n「我先表个态——我不是不让大家提想法，但这家店不是说改就改的。至少，我们得先想清楚，到底要守住什么。」\n\n**林澈**（身体微微前倾，语速偏快，像是已经等这句话很久了）：\n「那我先来。再拖下去店就真的没了。这两个月我们错过了至少两次机会——不是没人提方案，是一到关键处就卡住。」\n（他呼了一口气，压着火）\n「我不是来吵架的。但我觉得我们现在的问题不是缺想法，是已经拖不起了。」\n\n**阿宁**（原本靠在椅背上，这时慢慢坐直，语气带一点犹豫，但像忍了很久）：\n「……我插一句。你们说的改不改我不一定说得上话，但有一件事我想先说清楚——很多决定下来的时候，我是最后一个知道的。」\n（她语气不重，但话不绕）\n「上个月菜单要换，我当天下午四点半才收到通知。那天晚上我一个人在吧台加了三个小时班。这种事不是第一次了。」\n\n**许言**（双手交握放在桌上，先看一圈大家，语气尽量缓）：\n「我先不站队。我就说一个感觉——我刚才听下来，你们每个人说的都有道理，但你们好像不在说同一件事。」\n（他环视一圈）\n「林澈说的是机会，周岚说的是边界，阿宁说的是执行。你们自己有没有发现？」', '【你的任务卡】\n\n进入自由讨论。按照你角色剧本中的任务卡行动。\n任务完成后可解锁隐藏信息。\n\n说这些话会搞砸讨论：\n✕ 指责对方\n✕ 打断别人\n✕ 说"', { '周岚': '【你的第一轮任务】\n\n你心里对自己说：先别急着顶回去。先听他们说完。至少听出来两个人到底在担心什么——那种他们没直接说出来的担心。\n\n你可以这样开头：「你继续说，我听着。」\n\n注意说这些话会让讨论变糟：\n✕ 「你每次都是这种语气。」\n✕ 「那你觉得一定行吗？」', '林澈': '【你的第一轮任务】\n\n你对自己说：至少先搞清楚一件事——周岚说的「边界」，具体指什么。她说「这家店不是说改就改的」，但没说清楚哪些不能动。你问一个具体的问题，让她必须回答一个具体的点。\n\n你可以这样开头：「我不是说今晚就把所有事情定死，但你说的边界——能不能先讲一件你绝对不想动的？」\n\n注意说这些话会让讨论变糟：\n✕ 「你们太慢了。」\n✕ 「再这样下去就等死。」\n✕ 「讲这些有用吗？」', '阿宁': '【你的第一轮任务】\n\n你对自己说：把上个月菜单调整那天的情况完整说一遍。不只是「下午四点半才通知」——还要说清楚：谁通知的、你后来补了什么、补了多久。让他们听完再下判断。\n\n你可以这样开头：「不只是菜单那一件事。上个月那次具体是——」\n\n注意说这些话会让别人不再认真听你：\n✕ 「随便吧。」\n✕ 「反正最后都得我来收拾。」\n✕ 「我说了也没用。」', '许言': '【你的第一轮任务】\n\n你对自己说：今晚先摸清至少两个人对这家店的预期——他们各自说的「好」，到底是什么样子的。\n\n你可以这样开头：「我问个问题吧——在你们各自理想的画面里，一年后的621号店是什么样子的？」\n\n注意说这些话会让你失去观察者的中立位置：\n✕ 「你们先统一好了再找我。」\n✕ 「这是你们内部问题。」', '_hiddenInfo': [{ role:'周岚', tier:1, content:'前年那次快闪，我犹豫了很久才答应。一家品牌找上门提联合活动。活动期间人确实多了，但结束后——一个每天午后坐窗边看书的老先生，一个每周五带女儿来的单亲妈妈，再也没有出现过。后来在街上遇到老先生，他说"店里太吵了，后来习惯了去别家"。\n\n我难受的不是损失了两个客人。是我难得往前走了一步，没有陈栩在旁边——结果迈空了。\n\n从那以后我对所有"改动"都格外小心。不是因为保守，是因为我怕我一动，连现在仅剩的东西都会丢掉。' },{ role:'林澈', tier:1, content:'我第一次进这家店不是两年前。是2018年秋天。那天吧台后面的人不是周岚，是一个短头发、说话很快的人。她看我坐了很久什么都没点，放了一杯在我面前。"请你。喝完再说。"\n\n那个人叫陈栩。周岚从来没在我面前提过这个名字。我问过一次——我说"之前还有一个人"，她没接话。\n\n我帮这家店做海报、做账号、拉活动，不是因为我想证明什么。是因为那个人已经不在这里了，而这家店还在。我想帮它多撑一会儿。' },{ role:'阿宁', tier:1, content:'有一本笔记本我一直没给人看过。过去一年，每一次临时变动、紧急通知——我都记在上面了。不是因为记仇，是因为我在乎这家店怎么运转。\n\n3月12日菜单调整：林澈下午4:30发消息说"菜单换一下"，当晚我一个人补了三个小时。4月7日进货单：周岚上午发消息说"照新的来"，没人跟我说为什么换。5月20日活动物料：物料到了没人对接，我一个人从门口搬进来。\n\n我没拿出来，是因为拿出来像是在记账、像是在抱怨。但我心里清楚——如果我不拿出来，没人会真的知道执行端是什么情况。' },{ role:'许言', tier:1, content:'五年前我写那篇报道，不是因为那天下午我路过这里觉得舒服。是因为我表妹。她那年需要一笔紧急手术费。有人以"621号店"的名义转了一笔钱过来。\n\n我找到这家店，周岚指了指吧台角落一个旧的饼干铁盒，说："铁盒里刚好够。"\n\n这家店每卖一杯咖啡从利润里留一点放进那个铁盒。做了快十年。从没声张过。\n\n我写那篇三千字的报道——不是在记录一家咖啡馆。是在替一个受助过的家庭回一句谢谢。今晚我来，是来看那个铁盒还在不在吧台角落。' },]});
  insertScene(3, '第3环节：第一次讨论分享', '【讲师引导】\n\n组内讨论：\n\n• 你们觉得刚才谁是最想推动事情往前走的那个人？\n• 谁是最难被说服的？\n• 哪一句话让气氛明显变紧了？\n• 你们表面上在争什么？实际上在争什么？', 'discussion', '', '', null);
  insertScene(4, '第4环节 · 第2幕：门快关了', '', 'script', '**周岚**（语气比刚才沉了一些，像是在边想边说）：\n「刚才说到那份上，我觉得有些话确实得说开一点。我不是反对所有变化，但你们得理解——有些事我经历过，你们没有。」\n（她停了一下）\n「我不是说我有多了不起。但有些东西，不是说换就能换好的。」\n\n**林澈**（声音低了一点，不是在认输，是在说实话）：\n「我承认我有时候是急。但你们知道吗——我每次停下来等，最后就什么都没发生。我等过很多次了。」\n（他看了一眼所有人）\n「我不是在怪谁。我只是不想今晚谈完，一切又回到原样。」\n\n**阿宁**（顿了一下才开口，像是想了很久才决定要说）：\n「其实……我刚才说换菜单的事，只是其中一个。类似的事每个月都有。我不是不愿意配合，但每次都是最后通知、临时补——」\n（她停下来，过了一会儿）\n「你们有没有想过，哪天我要是不补了呢？」\n\n**许言**（语气平缓，但目光认真）：\n「我听到现在有一个感觉越来越明显——你们每个人都有道理，但你们在用自己的道理压对方的道理。」\n「我不是要分谁对谁错。我就想问一句——你们有没有谁，主动去问过别人『你心里真正担心的是什么』？」', '【你的任务卡】\n\n进入自由讨论。按照你角色剧本中的任务卡行动。\n任务完成后可解锁隐藏信息。\n\n说这些话会搞砸讨论：\n✕ 指责对方\n✕ 打断别人\n✕ 说"', { '周岚': '【你的第二轮任务】\n\n你对自己说：今晚至少要说一件我从来没提过的事。用事实说，不要用判断。我先开口，别人才可能跟我开口。\n\n你可以这样开头：「有件事我没跟你们提过。之前有一次做活动，后来其实——」\n\n注意说这些话会让讨论变糟：\n✕ 「你们根本不懂这家店。」\n✕ 「反正我不同意。」', '林澈': '【你的第二轮任务】\n\n你对自己说：今晚不管谁说什么，在反驳之前——先把他刚才说的用自己的话重复一遍。不是装客气，是确认我真的听懂了。\n\n你可以这样开头：「我先确认一下我有没有听岔——你说不能急着改，主要是因为——」\n\n如果有人问你为什么这么在意这家店，你可以说出2018年陈栩那件事。但不要抢着讲，等一个合适的问题。\n\n注意说这些话会让讨论变糟：\n✕ 「讲这些有用吗？」\n✕ 「你们就是想拖。」', '阿宁': '【你的第二轮任务】\n\n你对自己说：今晚把笔记本里的东西拿出来说。不用全给人看，但至少挑一件具体的事，让他们看到实际发生过什么。这是我今晚最有分量的一次开口。\n\n你可以这样开头：「我给你们看一条记录。上个月那次调整，具体是——」\n\n注意说这句话会让你错失机会：\n✕ 「算了，说了也没用。」', '许言': '【你的第二轮任务】\n\n你对自己说：今晚要让他们两个人直接回答对方的问题一次。不绕开，不岔开。我可能是今晚唯一一个能让这事发生的人。\n\n你可以这样开头：「等一下——林澈刚才问了一个问题。周岚你听到了吗？我想先听你回答他。」\n\n注意说这句话会让你失去推动作用：\n✕ 「算了，我大概知道了。」', '_hiddenInfo': [{ role:'周岚', tier:2, content:'这半年，店一直在亏。每个月我都在拿自己的钱往里填。到现在垫了有六万多。我算过，大概还能撑三个月。\n\n不是不想告诉他们——是说出来，就好像承认自己没经营好。我开这家店开了十二年，我不想让别人觉得我不行。' },{ role:'林澈', tier:2, content:'上个月有人挖我。一家新品牌咖啡店，管理岗。薪资高不少，月底要答复。\n\n我不是犹豫去不去——我是怕我走了之后，这家店撑不住。\n\n我甚至想过：如果今晚谈完还是老样子——那可能就是该走的时候了。' },{ role:'阿宁', tier:2, content:'上周有人来挖我了——另一家独立咖啡馆的店长岗。薪资高两成，权责比我在这边清楚得多。我还没回。我告诉自己说等今晚结果再说。\n\n但如果今晚还是各说各的、没有一个能落在纸上的说法——那我可能真的不需要再等了。' },{ role:'许言', tier:2, content:'几个月前我认识了这栋楼的房东。一次社区活动上聊到的。他对这家店印象不错——他女儿以前也喜欢来。但他也提到了：运营能力他不太确定。\n\n他话里留了空间——如果团队有清晰的方向和稳定的共识，租金上他可以松动。\n\n但这话我没跟店里任何人提过。因为我不会帮一个自己都还没想明白的团队。' },]});
  insertScene(5, '第5环节：第二次讨论分享', '【讲师引导】\n\n组内讨论：\n\n• 哪个角色最像早就憋了很久的样子？\n• 哪些问题其实不是今天晚上才有的？\n• 如果只是方案问题，为什么每次都谈不下来？\n• 你觉得真正卡住决定的到底是什么？', 'discussion', '', '', null);
  insertScene(6, '第6环节 · 第3幕（翻车版）：杯子还温着', '【场景说明】前一轮讨论结束后，其他人暂时离开。店里只剩周岚和林澈两个人。\n\n【讲师引导】需要一组周岚和林澈上台演绎。', 'script', '**林澈**（看着手机，快步走近，语速明显加快）：\n「刚收到消息。老街夜行活动那边还有一个位置。今晚就要回。」\n\n**周岚**（动作停住，慢慢抬头，皱眉）：\n「今晚就定？你不觉得太赶了吗？」\n\n**林澈**（把手机扣在桌上，压着不耐）：\n「机会不会等人。我们已经错过多少次了，你也知道。」\n\n**周岚**（站起一点又坐回去）：\n「我知道。但不是所有机会来了都得硬接。」\n\n**林澈**（往前一步，语气更重）：\n「现在的问题不是接得完不完美——是我们再这样下去，连试都没得试。」\n\n**周岚**（冷笑一下）：\n「你每次都这样。一有事就说赶紧做，好像不答应就是在拖后腿。」\n\n**林澈**（被顶到，声音压下来但更冲）：\n「那你呢？每次都再看看、再想想——到头来不还是什么都没定下来？」\n\n**周岚**（深吸一口气，语气变冷）：\n「所以现在变成是我的问题了，是吗？」\n\n**林澈**（不再控制）：\n「我没说是你的问题。但每次一到要决定的时候你就往后退——你让我怎么想？」\n\n**周岚**（身体坐直，盯着他）：\n「那是因为你根本没想清楚你要推的是什么。」\n\n**林澈**（彻底放弃，后退半步）：\n「行。那就继续这样吧。反正再拖一拖，也许店自己就有答案了。」', '', { '周岚': '【上台对戏版本】和林澈对戏。话头用完后自由发挥。任务：不说「不行」，说「什么情况下可以」。', '林澈': '【上台对戏版本】和周岚对戏。你先开口。任务：把为什么这么急的真实原因说出一件来。', '阿宁': '【观察任务】回答三个问题：1.他们有没有一次真的听懂了对方？2.哪一句换说法会不同？3.你会上台说什么？', '许言': '【观察任务】回答三个问题：1.他们争的是同一件事吗？2.谁的话里藏着真正怕的东西？3.你还有信心吗？', '_hiddenInfo': [{ role:'周岚', tier:3, content:'其实来之前我做了一个决定——如果今晚谈不拢，我就自己去找房东续约。哪怕只剩我一个人，我也不想让这家店就这么没了。\n\n但现在我有点不确定了。也许问题不是别人不努力——是我一直没把最真实的情况告诉他们。如果早点说，会不会不一样？我不知道。' },{ role:'林澈', tier:3, content:'老街夜行活动的机会是我托朋友拿到的。我跟他拍了胸脯说621号店一定参加——说那句话的时候我没跟任何人商量过。\n\n如果今晚退掉，丢的不只是一个机会。我朋友那边我没法交代。\n\n我也常常一急就不给人说话的机会。吵完之后我也会想：是不是又说过了。但下次一有压力——我还是先冲再说。那句「再拖就没了」不光说给他们听的，也是说给我自己的。' },{ role:'阿宁', tier:3, content:'说实话，我是真的在意这家店。要不是在意，我早就走了。\n\n但我最近越来越觉得——如果所有人都在说，但没人在听，那我的在意——就只是在替别人填坑。\n\n我今晚最想听到的，不是「改还是不改」。是有人认认真真问我一句：「你怎么想？」' },{ role:'许言', tier:3, content:'来之前我就有一个预期——可能会看到和五年前差不多的场面：每个人都在说，没人在听。\n\n我给自己定了一条线：如果今晚到第三幕结束之前，没有人主动去问别人一句「你真正担心的是什么」——那这家店，我觉得不值得再花力气了。\n\n但翻车版结束后，手机上出现了那个选择：公开，还是保留。那一刻的选择——是我对今晚的判断，也是我对这个团队的判断。' },]})
insertScene(7, '第7环节：观察分享 + 反转', '【讲师引导】\n\n1. 阿宁和许言分享观察发现。\n2. 全员讨论：他们表面上在争什么？实际上在争什么？\n3. 反转：讲师揭示身份——「我是十二年前投资这家店的人，从没露过面。」\n\n请所有人打开手机上还没解锁的信息。', 'discussion', '', '', null);
  insertScene(8, '第8环节 · 第3幕（修正版）：杯子还温着', '【讲师引导】同样的事，同样的人。但如果他们把真正重要的东西说清楚——会走向完全不同的结局。需要另一组周岚和林澈上台演绎。', 'script', '**林澈**（看着手机，先深呼吸一下放慢自己，再抬头，语速仍快但明显在控制）：\n「我刚收到消息。老街夜行活动那边还有一个位置。如果我们想参加，今晚必须先给一个答复。」\n\n**周岚**（没有立刻反驳，看着他，语气谨慎但平稳）：\n「你先继续说。你说的参加——具体是指今晚要定到什么程度？」\n\n**林澈**（像终于有人在问细节，神色松了一点）：\n「不用今晚把所有细节都定死。我的意思是——今晚先确定要不要争取这个机会。如果确定参加，我明天中午之前出一个简版方案，包括活动菜单、现场安排、宣传方式，我们再一起过。」\n\n**周岚**（神情稍微松一点，轻轻点头）：\n「好，这样说我比较能听明白。那我也先把话说清楚——我不是反对参加活动。我担心的是在准备还没到位的时候先答应下来，到最后为了赶时间，把店做得很失真。」\n\n**林澈**（没有急着反驳，而是先确认）：\n「你说的失真——对你来说最重要的，是哪一部分？」\n\n**周岚**（这次没有回避，直接说了）：\n「两个点。第一，不要为了活动把这家店原来的感觉全部打散。第二，不能像以前那样——执行压力到最后全落到阿宁一个人头上。这两条如果没有提前说清楚，我不会答应。」\n\n**林澈**（点头，认真重复确认）：\n「好，这两个点我记住了。那我也说清楚我的担心——我最怕的不是这次做不完美。我最怕的是如果连这种摆在眼前的机会我们都一直不去试，店会在反复的犹豫里慢慢失去所有可能性。」\n\n**周岚**（看着他，语气仍谨慎但明显没那么顶了）：\n「我能理解你为什么会急。我也不是想一直拖着不决定。但以前很多次讨论都停在先试再说，最后变成谁都很累，也说不清到底守住了什么。所以这次如果要试——我希望边界先定出来，再往前走。」\n\n**林澈**（立刻接住，语气里第一次有了合作感）：\n「可以。那我提一个具体的请求——今晚我们先确认是不是愿意参加；如果愿意，我明天出完整方案；出方案的时候你来帮我一起划清楚，哪些能动、哪些不能动。」\n\n**周岚**（想了两秒，点头）：\n「这个流程我可以接受。但要补两条。第一，方案出来之前不对外承诺任何细节。第二，阿宁要一起看执行表——不能是最后才通知她。」\n\n**林澈**（很快点头）：\n「没问题。对外我今晚只回复有意向参加，明天补具体方案。执行表我和阿宁一起列。如果明天你看完方案觉得触到了你的底线——我们保留不上这个活动的最终决定权。」\n\n**周岚**（终于真正松一口气，语气还是稳的）：\n「好，这样我能接受。那今晚先回有意向参加——但前提是明天中午之前我们三个人必须把方案从头到尾过一遍。」\n\n**林澈**（神色也缓下来了，补了一句）：\n「可以。我也补一句——如果我后面又说得太像只顾着往前推，你直接提醒我。我不想每次一着急，就不自觉把你推到对立面去。」\n\n**周岚**（看着他，沉了一下，也主动补上）：\n「那我也补一句——如果我只是说不行，没把具体的担心说清楚，你也可以直接问我到底是哪一条过不去。不然我们又回到以前那种谈法了。」\n\n**林澈**（轻轻笑了一下，不是开心，是终于有一点松了）：\n「行。那这次先不回到以前了。我先去回消息。」\n\n**周岚**（点头，语气很轻但明确）：\n「好。但回完消息——明天中午之前，一定要把方案说完整。」', '', { '周岚': '【修正版·上台】核心：把顾虑说清楚，而不是只说「不行」。注意动作标注。', '林澈': '【修正版·上台】核心：确认对方的底线，说出自己的担心。注意动作标注。', '阿宁': '【观察】对比翻车版和修正版，注意两个人分别做了哪些不同的事。', '许言': '【观察】修正版里哪些做法让对话从对抗变成了合作？', });
  insertScene(9, '第9环节：知识分享', '【三步动作】\n\n第一步 · 听关键\n对方话里那个让你不舒服的词，不要接，拆开问回去。\n"你说XX——具体是指哪件事？"\n\n第二步 · 说行为\n不辩解、不描述感受，描述当时实际发生了什么。\n"那次的具体情况是______。"\n\n第三步 · 定边界\n不说"我以后会改"，说"什么情况下我可以做到"。\n"可以______，如果______。你看行吗？"\n\n三步关联：听关键不炸，说行为说清，定边界往前走。', 'knowledge', '', '', null);
...
io.on('connection', (socket) => {
  ...
  // 诊断API：查看场次实时状态
app.get('/api/session/:code/diag', (req, res) => {
  const code = req.params.code;
  const session = stmts.getSessionByCode.get(code);
  if (!session) return res.json({ success: false, error: '场次不存在' });
  const scenes = stmts.getScenes.all(session.id);
  const participants = stmts.getParticipants.all(session.id);
  res.json({
    success: true,
    session: { id: session.id, code: session.code, current_scene: session.current_scene, status: session.status },
    sceneCount: scenes.length,
    participantCount: participants.length,
    participants: participants.map(function(p) { return { name: p.name, role: p.role_name, group: p.group_name }; })
  });
});
...
// 讲师推送剧本（HTTP 版，确保可靠）
  app.post('/api/session/:code/push-scene', (req, res) => {
    ...
    const code = req.params.code;
    console.log('[推送API] 收到请求 code=' + code + ' sceneNumber=' + sceneNumber);
    const session = stmts.getSessionByCode.get(code);
    if (!session) {
      console.log('[推送API] 场次不存在: ' + code);
      return res.status(404).json({ success: false, message: '场次不存在' });
    }
    console.log('[推送API] 当前current_scene=' + session.current_scene + ' 设为=' + sceneNumber);
    stmts.setCurrentScene.run(sceneNumber, code);
    if (session.status === 'preparing') {
      stmts.updateSessionStatus.run('active', code);
      console.log('[推送API] 状态从preparing改为active');
    }
    const scenes = stmts.getScenes.all(session.id).map(s => enrichSceneWithRoleContent(s, session.id));
    const pushedScenes = scenes.filter(s => s.scene_number <= sceneNumber);
    console.log('[推送API] 共' + scenes.length + '个场景，推送' + pushedScenes.length + '个');
    console.log('[推送API] 广播 session:' + code + ' room');
    io.to('session:' + code).emit('scene:pushed', {
      currentScene: sceneNumber,
      scenes: pushedScenes
    });
    console.log('[推送API] 广播完成');
    res.json({ success: true });
    res.json({ success: true });
  });
